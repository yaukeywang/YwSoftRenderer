...
namespace yw
{
    ...
    float4 DemoPBRIBLPixelShader::UNITY_BRDF_PBS(const float3& diffColor, const float3& specColor, const float3& lightColor, float oneMinusReflectivity, float smoothness, const float3& normal, const float3& viewDir, const float3& lightDir)
    {
...
#   endif
    ...
    float DemoPBRIBLPixelShader::DisneyDiffuse(float NdotV, float NdotL, float LdotH, float perceptualRoughness)
    {
